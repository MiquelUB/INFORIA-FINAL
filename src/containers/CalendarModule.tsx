...
const CalendarModule = () => {
  ...
  return (
    <div className="bg-card rounded-lg border border-border p-4 w-full h-full flex flex-col">
      {/* Title */}
            ...
            {day.date > 0 && (
              <>
                {day.hasAppointments && day.appointments ? (
                  <HoverCard>
                    <HoverCardTrigger asChild>
                      <button 
                        ...
                        className="w-full h-full flex items-center justify-center text-sm font-medium text-foreground hover:bg-accent rounded transition-colors relative bg-background border border-transparent hover:border-primary"
                      >
                        {day.date}
                        <div 
                          className="absolute top-1 right-1 w-2 h-2 rounded-full shadow-sm bg-red-600"
                        />
                      </button>
                    </HoverCardTrigger>
                      ...
                      <div className="space-y-2">
                        ...
                        {day.appointments.map((appointment, idx) => (
                          <div key={idx} className="text-xs p-2 bg-background rounded border">
                            ...
                            <button 
                              onClick={() => navigate('/patients/1/detailed')}
                  ...
                  <button 
                    onClick={() => handleDayClick(day.date)}
                    className="w-full h-full flex items-center justify-center text-sm font-medium text-foreground hover:bg-accent rounded transition-colors bg-background border border-transparent hover:border-muted"
                  >
                    {day.date}
                  </button>
                )}
