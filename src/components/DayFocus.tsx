...
const DayFocus = () => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "Pendiente":
        return "bg-yellow-500"; // Dorado Premium para citas pendientes
      case "En Curso":
        return "bg-red-600"; // Burdeos Acento para citas en curso
      case "Completada":
        return "bg-green-600"; // Verde INFORIA para citas completadas
      default:
        return "bg-gray-500";
    }
  };
  ...
  return (
    <div className="bg-card rounded-lg border border-border p-8 w-full max-w-4xl mx-auto">
      {/* Title */}
      ...
      {displayedAppointments.length > 0 ? (
        <div className="space-y-6">
          {displayedAppointments.map((appointment) => (
            <div 
              key={appointment.id}
              className="flex items-center justify-between p-6 bg-background rounded-lg border border-border transition-colors hover:shadow-md"
            >
              <div className="flex items-center space-x-4">
                ...
                <div className="text-xl font-semibold text-foreground min-w-[70px]">
                 ...
                 {/* Patient Name */}
                <Link 
                  to={`/patients/${appointment.id}/detailed`}
              ...
              {/* Action Button */}
              <Link to={`/session/${appointment.id}`}>
